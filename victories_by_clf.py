...
for measure in ['roc_aucs', 'f1_scores', 'g_means']:
    for country in ["polish", "russian", "taiwanese"]:
        ...
        for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
            ...
            for switch in switches:
                for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]: 
                    victories[(fs, blc, switch)] = {}
                    for percentile in percentiles:
                        victories[(fs, blc, switch)][percentile] = defaultdict(int)
                    
                        for fold, (train_idx, test_idx) in enumerate(joblib.load(f'cvs_{country}.jbl')['10 fold splits']): #10
                            name2 = constructName2(fold, percentile, fs, blc, clf, switch)
                            tmpl = vs[name2].split("_") # '6_Fold0_Pct10_RandomForest_---_RandomForestClassifier_True'
                            val = tmpl[-2] 
                            victories[(fs, blc, switch)][percentile][val] += 1

                        for clf, clmethod in clmethods:
                            if clf not in victories[(fs, blc, switch)][percentile]:                        
                                victories[(fs, blc, switch)][percentile][clf] = 0

        # The area for a colour corresponding to a fs-bal combination
        # denotes the number of victories for that combination.
        #
        graph = {}
        for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
            if blc is None:
            ...
            for switch in switches:
                for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]: 
                    ...
                    for percentile in percentiles:
                        graph[(fs, blc, switch)][percentile] = {}
                        keys = [
                        ...
                        for i, key in enumerate(keys):
                            if i:
                                graph[(fs, blc, switch)][percentile][key] = victories[(fs, blc, switch)][percentile][key] + graph[(fs, blc, switch)][percentile][keys[i-1]]
                            else:
                                graph[(fs, blc, switch)][percentile][key] = victories[(fs, blc, switch)][percentile][key]
        ...
        zpairs = [
            ('Z1', 'RandomForestClassifier'),
            ('Z2', 'LinearSVC'),
            ('Z3', 'SVC'),
            ('Z4', 'GradientBoostingClassifier'),
            ('Z5', 'KNNClassifier'),
            ('Z6', 'NaiveBayes'),
            ('Z7', 'CART'),
        ]
        ...
        Xdict_new = {k: Xdict[k] for k in X}
            ...
            for y in Y:
                for k in percentiles:
                    for i in range(1, 8):
                        ...
                        # x - fs, y - blc, switch
                        data[(x, y)][k][z] = graph[(fs, blc, switch)][percentile][key]


        # ----- Step 2. Plot setup -----
        fig, axes = plt.subplots(len(Y), len(X), figsize=(18, 18), sharex=True, sharey=True)

        colors = plt.cm.cividis(np.linspace(0.1, 0.9, 7))
        ...
        for j, y in enumerate(Y):
            for i, x in enumerate(X):
                ...
                # Optional: overlay lines for each v_i
        ...
        def getLegendLabel(inLabel):
            if inLabel == "RandomForestClassifier":
                return "Random Forest"
            if inLabel == "LinearSVC":
                return "Linear SV"
            if inLabel == "SVC":
                return "SV"
            if inLabel == "GradientBoostingClassifier":
                return "Gradient Boosting"
            if inLabel == "KNNClassifier":
                return "kNN"
            if inLabel == "NaiveBayes":
                return "Naive Bayes"
            return inLabel

        legend_handles = [
            mpatches.Patch(color=colors[i], label=f"{getLegendLabel(zpairs[i][1])}") for i in range(7)
        ]
        ...
        fig.supxlabel("Feature Selector", y=0.05, fontsize=16)
        fig.supylabel("Feature Selector-Resampler Combination", fontsize=16, x=0.0, va='center')
        plt.tight_layout(rect=[0, 0.05, 1, 0.96])
        
        # Save figure before showing
        plt.savefig(
            f"./charts/{country}/victory3_vs_features_{country}_{measure[:-1]}.png",
