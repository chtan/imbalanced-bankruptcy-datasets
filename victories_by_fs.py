...
for measure in ['roc_aucs', 'f1_scores', 'g_means']:
    for country in ["polish", "russian", "taiwanese"]:
        ...
        # Find the fs that is most performant - the victor -
        # given clf, bal/switch combo, pct
        vs = {}
                    ...
                    for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
                        ...
                        for switch in switches:
                            ...
                            for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]:
                                name = constructName(fold, percentile, fs, blc, clf, switch)
                                tmpd[name] = m[name]['roc_aucs'][0]

                            max_key = max(tmpd, key=tmpd.get) # max for this percentile, fold
        ...
        for clf, clmethod in clmethods:
            for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
                if blc is None:
                    ...
                    switches = [True, False]
                for switch in switches:
                    victories[(blc, switch, clf)] = {}
            
                    for percentile in percentiles:
                        victories[(blc, switch, clf)][percentile] = defaultdict(int)
                    
                        for fold, (train_idx, test_idx) in enumerate(joblib.load(f'cvs_{country}.jbl')['10 fold splits']): #10

                            name2 = constructName2(fold, percentile, fs, blc, clf, switch) # ('Fold4', 'Pct20', 'Anova', 'CART')
                            tmpl = vs[name2].split("_") # '6_Fold0_Pct10_RandomForest_---_RandomForestClassifier_True'
                            val = tmpl[3]
                            victories[(blc, switch, clf)][percentile][val] += 1

                        for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]: 
                            if fs not in victories[(blc, switch, clf)][percentile]:
                                victories[(blc, switch, clf)][percentile][fs] = 0
        ...
        for clf, clmethod in clmethods:
            for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
                if blc is None:
                    ...
                    switches = [True, False]
                for switch in switches:
                    graph[(blc, switch, clf)] = {}
            
                    for percentile in percentiles:
                        graph[(blc, switch, clf)][percentile] = {}
                        keys = ['Anova', 'Info', 'LogisticRegressionWitL1Penalty', 'RandomForest', 'PSO', 'GA']
                        for i, key in enumerate(keys):
                            if i:
                                graph[(blc, switch, clf)][percentile][key] = victories[(blc, switch, clf)][percentile][key] + graph[(blc, switch, clf)][percentile][keys[i-1]]
                            else:
                                graph[(blc, switch, clf)][percentile][key] = victories[(blc, switch, clf)][percentile][key]
        ...
        ypairs = [
            (f'Y{i}', clmethods[i-1][0])
            for i in range(1, 8)
        ]

        fsmethods = score_funcs[:-1] + fsmethods_embedded + fsmethods_wrapper

        zpairs = [
            ('Z1', 'Anova'),
            ('Z2', 'Info'),
            ('Z3', 'LogisticRegressionWitL1Penalty'),
            ('Z4', 'RandomForest'),
            ('Z5', 'PSO'),
            ('Z6', 'GA'),
        ]
        ...
        blcombos = [
            ...
            ('RandomUnderSampler', 'True'), ('SMOTE', 'False'), 
            ('SMOTE', 'True')
        ]
        blcombos_words = [
            "FS",
        ...
        for x in X:
            for y in Y:
                subdict = {}
                base = np.zeros_like(percentiles, dtype=float)
                for k in percentiles:
                    # Simulate increasing v-values
                    v = np.cumsum(np.sort(rng.random(7)))  # monotonic increasing randoms
                    v = 10 * v / v[-1]  # scale to [0, 10]
            ...
            for y in Y:
                for k in percentiles:
                    for i in range(1, 7):
                        ...
                        # x - blc, switch; y - clf
                        data[(x, y)][k][z] = graph[(blc, switch, clf)][percentile][key]


        # ----- Step 2. Plot setup -----
        fig, axes = plt.subplots(7, 7, figsize=(18, 18), sharex=True, sharey=True)
        ...
        for j, y in enumerate(Y):
            for i, x in enumerate(X):  
                ...
                # Build arrays: v1,...,v7 over percentiles
                v_arrays = []
                for zi in range(1, 7):
                    v_arrays.append([subdict[k][f"Z{zi}"] for k in percentiles])
                v_arrays = np.array(v_arrays)

                # Fill between curves
                ax.fill_between(percentiles, 0, v_arrays[0], color=colors[0], alpha=0.6)
                for zi in range(6):
                    ax.fill_between(percentiles, v_arrays[zi-1], v_arrays[zi],
                                    color=colors[zi], alpha=0.6)

                # Overlay lines for each v_i
                ax.plot(percentiles, v_arrays.T, color='black', lw=0.5, alpha=0.7)
                ...
                if j == 6:
                    label = Xdict[x]
                    if label == "FS + RandomOverSampler":
                        label = "FS+ROS"
                    elif label == 'FS + RandomUnderSampler':
                        label = 'FS+RUS'
                    elif label == 'RandomOverSampler + FS':
                        label = 'ROS+FS'
                    elif label == 'RandomUnderSampler + FS':
                        label = 'RUS+FS'
                    elif label == 'SMOTE + FS':
                        label = 'SMOTE+FS'
                    elif label == 'FS + SMOTE':
                ...
                if i == 0:
                    ...
                    if label == "NaiveBayes":
                        ...
                        label = 'SV'
                    elif label == 'RandomForestClassifier':
                        label = 'Random Forest'
                    ax.set_ylabel(label)
        ...
        # ----- Step 3. Add shared legend -----
        def getLegendLabel(inLabel):
            if inLabel == "Anova":
                #return "t-Test"
                return "Anova"
            if "LogisticRegression" in inLabel:
                return "Logistic Regression"
            if inLabel == "RandomForest":
        ...
        legend_handles = [
            mpatches.Patch(
                color=colors[i],
                label=getLegendLabel(zpairs[i][1])
            )
            for i in range(6)
        ]

        # place legend on the right side, outside the subplots
        fig.legend(
        ...
        fig.supxlabel("Feature Selector-Resampler Combination", y=0.05, fontsize=16)
        fig.supylabel("Classifier", fontsize=16, x=0.0, va='center')
        plt.tight_layout(rect=[0, 0.05, 1, 0.96])

        # Save figure before showing
        plt.savefig(
            f"./charts/{country}/victory2_vs_features_{country}_{measure[:-1]}.png",
