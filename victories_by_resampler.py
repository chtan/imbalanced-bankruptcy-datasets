...
for measure in ['roc_aucs', 'f1_scores', 'g_means']:
    for country in ["polish", "russian", "taiwanese"]:
            ...
            for percentile in percentiles:
                for clf, clmethod in clmethods:
                    for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]:
                        # collects the names and their performance metric, for given country, percentile, fold, clf, fs
                        tmpd = {} 
                        ...
                        for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
                            ...
                            for switch in switches:
                                name = constructName(fold, percentile, fs, blc, clf, switch)
                                tmpd[name] = m[name][measure][0] # read the roc_auc score for this name

                        max_key = max(tmpd, key=tmpd.get) # max for this given percentile, fold, clf, fs
                        name2 = constructName2(fold, percentile, fs, blc, clf, switch) # drops/hides blc and switch

                        vs[name2] = max_key


        # Count the number of times each combination is victorious
        victories = {}
        for clf, clmethod in clmethods:
            for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]:
                victories[(fs, clf)] = {}
                for percentile in percentiles:
                    victories[(fs, clf)][percentile] = defaultdict(int)
                    for fold, (train_idx, test_idx) in enumerate(joblib.load(f'cvs_{country}.jbl')['10 fold splits']):
                        name2 = constructName2(fold, percentile, fs, blc, clf, switch)

                        tmpl = vs[name2].split("_")
                        blc = tmpl[-3]
                        switch = tmpl[-1]
                        ...
                        switch = eval(switch)
                        victories[(fs, clf)][percentile][(str(blc), str(switch))] += 1
                    ...
                    for blc in ["SMOTE", "RandomOverSampler", "RandomUnderSampler", None]:
                        ...
                        for switch in switches:
                            if (str(blc), str(switch)) not in victories[(fs, clf)][percentile]:
                                victories[(fs, clf)][percentile][(str(blc), str(switch))] = 0
            ...
            for fs in ["Anova", "Info"] + ["LogisticRegressionWitL1Penalty", "RandomForest"] + ["PSO", "GA"]:
                ...
                for percentile in percentiles:
                    ...
                    for i, key in enumerate(keys):
                        if i:
                            graph[(fs, clf)][percentile][key] = victories[(fs, clf)][percentile][key] + \
                                                                graph[(fs, clf)][percentile][keys[i-1]]
                        else:
                            graph[(fs, clf)][percentile][key] = victories[(fs, clf)][percentile][key]
        ...
        data = {}
        # creates a NumPy random number generator using the newer Generator API
            ...
            for y in Y:
                for k in percentiles:
                    for i in range(1, 8):
                        ...
                        # x - clf, y - fs, z - key
                        data[(x, y)][k][z] = graph[(fs, clf)][percentile][key]


        # ----- Step 2. Plot setup -----
        Y_new = [y for j, y in enumerate(Y)]
        Ydict_new = {k: Ydict[k] for k in Y_new}
        fig, axes = plt.subplots(len(Y_new), len(X), figsize=(18, 16), sharex=True, sharey=True)

        colors = plt.cm.viridis(np.linspace(0.1, 0.9, 7))  # 7 shades
        ...
        plt.savefig(
            f"./charts/{country}/victory1_vs_features_{country}_{measure[:-1]}.png",
